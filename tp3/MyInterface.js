...
export class MyInterface extends CGFinterface {
  ...
  init(application) {
    ...
    this.gui.add(this.scene, 'selectedMaterial', this.scene.materialIDs)
    ...
    sf2.add(this.scene.lights[1], 'constant_attenuation', 0.00, 1.00)
        .name('Const. Atten.');
    sf2.add(this.scene.lights[1], 'linear_attenuation', 0.0, 1.0)
        .name('Linear Atten.');
    sf2.add(this.scene.lights[1], 'quadratic_attenuation', 0.0, 1.0)
        .name('Quad. Atten.');

    // Anothe forlder for grouping the custom material's parameters
    var f2 = this.gui.addFolder('Custom Material');

    f2.addColor(this.scene.customMaterialValues, 'Ambient')
        .onChange(this.scene.updateCustomMaterial.bind(this.scene));
    f2.addColor(this.scene.customMaterialValues, 'Diffuse')
        .onChange(this.scene.updateCustomMaterial.bind(this.scene));
    f2.addColor(this.scene.customMaterialValues, 'Specular')
        .onChange(this.scene.updateCustomMaterial.bind(this.scene));
    f2.add(this.scene.customMaterialValues, 'Shininess', 0, 100)
        .onChange(this.scene.updateCustomMaterial.bind(this.scene));
