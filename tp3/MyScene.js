...
export class MyScene extends CGFscene {
  ...
  init(application) {
    ...
    this.objects = [
      this.plane, this.pyramid, this.cone, this.tangram, this.cube, this.prism, this.cylinder
    ];
  ...
  initCameras() {
    this.camera = new CGFcamera(
        0.4, 0.1, 500, vec3.fromValues(10, 10, 10), vec3.fromValues(0, 0, 0));
  }
  ...
  hexToRgbA(hex) {
    ...
    if (/^#([A-Fa-f0-9]{3}){1,2}$/.test(hex)) {
      ret = [
        ...
        parseInt(hex.substring(5, 7), 16).toPrecision() / 255.0, 1.0
      ];
    } else
      ret = [
        hex[0].toPrecision() / 255.0, hex[1].toPrecision() / 255.0,
        hex[2].toPrecision() / 255.0, 1.0
      ];
    return ret;
  }
