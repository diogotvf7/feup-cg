...
export class MyBeeBody extends CGFobject {
  constructor(
      scene, slices, stacks, radius = 1, normals_direction = 1) {
    super(scene);
  ...
  normal(x, y, z) {
    ...
    // Apply direction
    return [
      this.normals_direction * x, this.normals_direction * y,
      this.normals_direction * z
    ];
  }
  ...
  initBuffers() {
    ...
    for (let i = 0; i < this.stacks; i++) {
      for (let j = 0; j < this.slices + 1; j++) {
        ...
        const x = this.radius * Math.cos(stack_angle) * Math.sin(slice_angle) * Math.pow(i, 0.7) / Math.pow(this.stacks, 0.7);
        const y = this.radius * Math.sin(stack_angle);
        const z = this.radius * Math.cos(stack_angle) * Math.cos(slice_angle) * Math.pow(i, 0.7) / Math.pow(this.stacks, 0.7);
    ...
    for (let i = 0; i < this.stacks - 1; i++) {
      for (let j = 0; j < this.slices; j++) {
        ...
        const next_vertex =
            i * (this.slices + 1) + (j + 1) % (this.slices + 1) + 2;

        const next_stack_vertex = (i + 1) * (this.slices + 1) + j + 2;
        const next_stack_next_vertex =
            (i + 1) * (this.slices + 1) + (j + 1) % (this.slices + 1) + 2;
