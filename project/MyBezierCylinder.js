...
export class MyBezierCylinder extends CGFobject {
  constructor(scene, radius, axis, radius_coefficient, slices = 20, stacks = 20) {
    ...
    this.radius = radius;
    this.axis = axis;
    this.radius_coefficient = radius_coefficient ?? [
      [1, 1], [1, 1], [1, 1], [1, 1]
    ];
    this.slices = slices;
    this.stacks = stacks;
    ...
    for (let i = 0; i <= this.stacks; i += 1) {
      ...
      for (let j = 0; j < this.slices; j++) {
        ...
        if (i < this.stacks) {
          ...
          console.log(offset + (j + 1) % this.slices, offset + j, offset + this.slices);
          this.indices.push(offset + (j + 1) % this.slices, offset + j, offset + this.slices - 1);
        } if (i == 0) {
          console.log(x, y, z);
          this.indices.push(0, offset + j, offset + (j + 1) % this.slices);
        }
      }      
    }
